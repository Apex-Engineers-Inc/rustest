...
mod tests {
    ...
    fn run_discovery(py: Python<'_>, path: &Path) -> Vec<crate::model::TestModule> {
        let config = RunConfiguration::new(None, None, None, true, true, LastFailedMode::None, false);
    ...
    fn test_pattern_filtering() {
        Python::with_gil(|py| {
            ...
            let config =
                RunConfiguration::new(Some("nonexistent".to_string()), None, None, true, true, LastFailedMode::None, false);
            let paths = PyPaths::from_vec(vec![file_path.to_string_lossy().into_owned()]);
            let modules = discover_tests(py, &paths, &config).expect("discovery should succeed");
    ...
    fn test_execution_with_capture_output_disabled() {
        Python::with_gil(|py| {
            ...
            let config = RunConfiguration::new(None, None, None, false, true, LastFailedMode::None, false);
            let paths = PyPaths::from_vec(vec![file_path.to_string_lossy().into_owned()]);
            let modules = discover_tests(py, &paths, &config).expect("discovery should succeed");
            let report =
                run_collected_tests(py, &modules, &config).expect("execution should succeed");
    ...
    fn test_worker_count_configuration() {
        let config1 = RunConfiguration::new(None, None, Some(1), true, true, LastFailedMode::None, false);
        assert_eq!(config1.worker_count, 1);

        let config2 = RunConfiguration::new(None, None, Some(8), true, true, LastFailedMode::None, false);
        assert_eq!(config2.worker_count, 8);

        let config3 = RunConfiguration::new(None, None, None, true, true, LastFailedMode::None, false);
