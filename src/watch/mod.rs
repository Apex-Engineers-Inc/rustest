...
use crate::model::{LastFailedMode, PyRunReport, RunConfiguration};
...
pub fn watch_mode(
    ...
    let config = RunConfiguration::new(
        ...
        workers,
        capture_output,
        false, // enable_codeblocks (not applicable in watch mode)
        LastFailedMode::None,
        false, // fail_fast
        false, // pytest_compat
        false, // verbose (watch mode has its own output)
        false, // ascii
        false, // no_color
    );
