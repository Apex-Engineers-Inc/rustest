...
| `mutate_p1b2_task5.py` | P1b.2 Task 5 §8 — the 30-row mutation table for the `--v2-run` gate |
...
Both `mutate_*.py` scripts **edit tracked source files in place** and restore them after
...
The two tables aim at different targets, which is worth keeping straight: Task 4's mutates
**the runner** (`src/v2/*`, `python/rustest/*`) and asks whether the tests would notice a
behaviour change; Task 5's mutates **the conformance harness** and asks whether the gate
would notice if it stopped grading something. The gate itself answers the first question
for the run surface, so duplicating it there would be circular.
