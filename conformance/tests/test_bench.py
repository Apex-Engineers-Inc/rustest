...
def _per_file_row(
    ...
    rustest_v2_collect_s: float = 0.0,
    tests: int = 5000,
) -> PerFileRow:
    ...
    return {
        ...
        "pytest_run_s": pytest_run_s,
        "rustest_v2_run_s": rustest_v2_run_s,
        "rustest_v2_collect_s": rustest_v2_collect_s,
    }
...
def test_run_benchmarks_quick() -> None:
    ...
    assert per_file["pytest_run_s"] > 0 and per_file["rustest_v2_run_s"] > 0
    assert per_file["rustest_v2_collect_s"] > 0
...
def test_derive_per_file_is_the_slope_between_the_two_cells() -> None:
    ...
    derived = derive_per_file(
        [
            _per_file_row(100, pytest_run_s=4.0, rustest_v2_run_s=5.0, rustest_v2_collect_s=0.2),
            _per_file_row(500, pytest_run_s=20.0, rustest_v2_run_s=21.0, rustest_v2_collect_s=0.6),
        ]
    )
    ...
    assert derived["rustest_v2_per_file_ms"] == pytest.approx(40.0)
    assert derived["rustest_v2_per_file_collect_ms"] == pytest.approx(1.0)
    ...
    assert derived["rustest_v2_overhead_us_per_test"] is None
...
def test_derive_per_file_refuses_cells_that_differ_in_tests() -> None:
    ...
    assert mixed["rustest_v2_per_file_ms"] is None
