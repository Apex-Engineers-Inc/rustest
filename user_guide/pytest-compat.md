...
| **Markdown code blocks** | rustest collects python fences out of a `.md` file **named as an argument**; a *directory* walk collects none. | `rustest user_guide/` runs nothing, so name the files: `rustest user_guide/*.md`. pytest has no equivalent without a plugin, which is why the walk does not. |
