...
::: {.callout-warning title="A session-scoped async fixture does not pull the test onto its loop"}
...
::: {.callout-warning title="`asyncio_mode` is the one default that differs from pytest-asyncio"}
