...
[AUDIT ADJUDICATED 2026-07-27: KEEP marshmallow/rich/Werkzeug/sqlparse; SWAP networkx→cachetools (load-bearing 55-file collect_ignore) and Pygments→humanize (custom collector classes); dateutil KEPT over the auditor's mechanical flag (single shallow modifyitems = the user's "limited ways" case; delta ledgered; hypothesis in its own venv). USER ADDITION: **PyniteFEA** (local read-only tree, audited zero-hooks/zero-config, unittest-heavy `Testing/` package — config drafted at conformance/real/pynite.toml). Slate = EIGHT new suites; twelve total in the final table.] [EXPANDED AGAIN 2026-07-27, user-approved axis fills: **attrs** (hypothesis ecosystem), **FastAPI** (modern async web), **Pillow** (C-extension testing), **jsonschema** (data-driven mega-collection), **psutil** (platform-skip + xunit-style setup probing). Hook audit before download as always. Slate = THIRTEEN new; SEVENTEEN total.] [BATCH-2 AUDIT ADJUDICATED: attrs/Pillow keep with ledgered limited hooks; jsonschema/psutil clean. FastAPI: USER DECIDED partial run — @anyio-marked tests deselected (anyio's pytest11 collection plugin is the blocker, not FastAPI); sync TestClient majority runs; async subset ledgered plugin-dependent. Tag-SHA pins re-verified at clone (api.github.com was 403 during audit).]
...
- Wall-clock table for all seventeen suites goes to the report (README numbers come from this table in Task 3).
