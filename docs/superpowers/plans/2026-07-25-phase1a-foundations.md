...
- Contract: `build_nodeid("tests/test_a.py", &["TestBox", "test_m"], Some("x-1"))` == `"tests/test_a.py::TestBox::test_m[x-1]"`; no escaping is performed (param ids arrive pre-formed; pytest's own id sanitization happens at id-generation time in 1b, not here); `split_nodeid` splits path at the **first** `::` and param at the first `[` in the remainder, tolerating `::` never appearing (bare file id). [AMENDED after Task 4 review: the original "first `[` after the last `::`" rule mis-splits pytest-emitted param ids containing `::` (e.g. `test_top[a::b]`); the shipped rule weakly dominates it on every input class and matches `CollectedTest.path`'s "first segment" contract. Note `Some("")` IS pytest-reachable: `parametrize("s", [""])` emits `test_x[]`.]
